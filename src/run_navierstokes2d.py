...
def get_parser():
    ...
    parser.add_argument("--dataset_name", type=str, default="navierstokes2d")
    parser.add_argument(
        "--data-path",
        type=str,
        default="./datasets/NavierStokes2D/ns_V1e-3_N5000_T50.mat",
    )
    ...
    parser.add_argument(
        "--in-channels", type=int, default=12
    )  # previous 10 frames + x + y
    parser.add_argument("--out-channels", type=int, default=1)  # next-step prediction
    parser.add_argument(
...
def build_loaders(args):
    ...
    train_set = NavierStokes2DDataset(split.train_x, split.train_y)
    val_set = NavierStokes2DDataset(split.val_x, split.val_y)
...
def _rollout_autoregressive(model, x, y, rel_loss_fn, step, t_in, use_coord):
    coeff_channels = t_in
    coord_channels = x[..., coeff_channels:] if use_coord else None
    state = x.clone()

    loss = 0.0
    step_rel_meter = Monitor()
    pred_steps = []
    ...
    for t in range(0, total_out, step):
        ...
        loss = loss + rel_loss_fn(pred_step, y_step)
        step_rel_meter.update(rel_loss_fn(pred_step, y_step).item(), x.shape[0])
    ...
    pred_full = torch.cat(pred_steps, dim=-1)
    full_rel = rel_loss_fn(pred_full, y)
    return loss, step_rel_meter.avg, full_rel, pred_full


def validate(model, loader, mse_loss_fn, rel_loss_fn, device, args):
    model.eval()

    loss_meter = Monitor()
    mse_meter = Monitor()
    rel_meter = Monitor()
    rel_step_meter = Monitor()
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            ar_loss, step_rel, full_rel, pred = _rollout_autoregressive(
                model=model,
                x=x,
                y=y,
            ...
            loss = 1.0 * mse_loss + 1.0 * ar_loss

            batch_size = x.shape[0]
            loss_meter.update(loss.item(), batch_size)
            mse_meter.update(mse_loss.item(), batch_size)
            rel_meter.update(full_rel.item(), batch_size)
            rel_step_meter.update(step_rel, batch_size)

    return {
        "loss": loss_meter.avg,
        "mse": mse_meter.avg,
        "rel_l2": rel_meter.avg,
        "rel_l2_step": rel_step_meter.avg,
    }
...
def main():
    ...
    if args.t_out % args.step != 0:
        raise ValueError(
            f"t_out must be divisible by step, got t_out={args.t_out}, step={args.step}"
        )

    expected_in_channels = args.t_in + (0 if args.no_coord else 2)
    if args.in_channels != expected_in_channels:
        raise ValueError(
            f"For Navier-Stokes autoregressive setup, in_channels should be t_in + coord_dims = {expected_in_channels}, got {args.in_channels}"
        )
    if args.out_channels != args.step:
        raise ValueError(
            f"For next-step autoregression, out_channels should equal step = {args.step}, got {args.out_channels}"
        )
    ...
    # Time-dependent 2D Navier-Stokes with autoregressive rollout
    ...
    for epoch in range(args.epochs):
        ...
        for step_idx, (x, y) in enumerate(train_loader):
            ...
            batch_size = x.shape[0]
            loss_meter.update(loss.item(), batch_size)
            error_meter.update(full_rel.item(), batch_size)
            error_step_meter.update(step_rel, batch_size)
            ...
            if global_step % args.log_interval == 0:
                ...
                logger.info(
                    ...
                    f"StepError: {error_step_meter.avg:.4e} "
                    f"RolloutError: {error_meter.avg:.4e} "
        ...
        logger.info(
            ...
            f"TrainStepError: {error_step_meter.avg:.4e} "
            f"TrainRolloutError: {error_meter.avg:.4e} "
            f"ValLoss: {val_metrics['loss']:.4e} "
            f"ValStepError: {val_metrics['rel_l2_step']:.4e} "
            f"ValRolloutError: {val_metrics['rel_l2']:.4e} "
            f"LR: {current_lr:.4e} "
